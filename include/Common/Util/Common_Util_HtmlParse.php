...
class Common_Util_HtmlParse
{
	...
	function findFirstLt($content, $start, &$gtpos)
	{
		...
		{
			return Common_Util_HtmlParse::findFirstLt($content, $start + 1, $gtpos);
		}
	}

	function findLastGt($content, &$ltpos)
	{
		...
		{
			return Common_Util_HtmlParse::findLastGt($content, $ltpos);
		}
	}

	function html2hbt($content)
	{
		$start = Common_Util_HtmlParse::findFirstLt($content, 0, $gtpos);
		$end = Common_Util_HtmlParse::findLastGt($content, $ltpos);
		...
		return array("head" => trim(substr($content, 0, $start)), "t1" => substr($content, $start, $gtpos - $start + 1), "body" => Common_Util_HtmlParse::html2hbt($body), "t2" => substr($content, $ltpos, $end - $ltpos + 1), "tail" => trim(substr($content, $end + 1)));
	}
	...
	function tag2tree($tag, &$tree, &$taglist)
	{
		list($tagname, $begin) = Common_Util_HtmlParse::getTagName($tag);
		if (Common_Util_HtmlParse::isPair($tagname))
		{
			if ($begin)
			{
				$taglist[] = array(
		...
		else if (Common_Util_HtmlParse::isSingle($tagname))
		{
			if ($begin)
			{
				if ($taglist)
	...
	function _html2tree($hbt, &$tree, &$taglist)
	{
		if (strlen($hbt["head"]))
		{
			$len = count($taglist);
		...
		if (strlen($hbt["t1"]))
		{
			Common_Util_HtmlParse::tag2tree($hbt["t1"], $tree, $taglist);
		}
		if ($hbt["body"])
		{
			Common_Util_HtmlParse::_html2tree($hbt["body"], $tree, $taglist);
		}
		if (strlen($hbt["t2"]))
		{
			Common_Util_HtmlParse::tag2tree($hbt["t2"], $tree, $taglist);
		}
		if (strlen($hbt["tail"]))
		{
			$len = count($taglist);
	...
	function html2tree($content)
	{
		$hbt = Common_Util_HtmlParse::html2hbt($content);
		...
		$taglist = array();
		Common_Util_HtmlParse::_html2tree($hbt, $tree["content"], $taglist);
		$tree["content"] = array_merge($tree["content"], $taglist);

		return $tree;
	}

	function pair2html($item, $length, $ext, &$taglist, &$html, &$txtlen, &$cut)
	{
		if ($txtlen >= $length)
		{
			return;
		...
		foreach ($item["content"] as $v)
		{
			if ($v["type"] == "pair")
			{
				Common_Util_HtmlParse::pair2html($v, $length, $ext, $taglist, $html, $txtlen, $cut);
			}
			else if ($v["type"] == "single")
			{
				Common_Util_HtmlParse::single2html($v, $length, $ext, $taglist, $html, $txtlen, $cut);
			}
			else if ($v["type"] == "text")
			{
				Common_Util_HtmlParse::text2html($v, $length, $ext, $taglist, $html, $txtlen, $cut);
			}
			if ($txtlen >= $length)
			{
				$cut = true;
	...
	function single2html($item, $length, $ext, &$taglist, &$html, &$txtlen, $cut)
	{
		$html .= $item["begin"];
	}

	function text2html($item, $length, $ext, &$taglist, &$html, &$txtlen, $cut)
	{
		$html .= Common_Util_Str::subString($item["content"], $length - $txtlen, $ext);
		$txtlen += strlen($item["content"]);
	}

	function getAbstract($content, $length, $ext = "", $dropBadTags = false)
	{
		$len = strlen($content);
		$subcut = false;
		if($len > $length * 10)
		{
			$content = Common_Util_Str::subString($content, $length*10);
			$subcut = true;
		}
		$tree = Common_Util_HtmlParse::html2tree($content);
		$html = "";
		$txtlen = 0;
		$taglist = array();
		$cut = false;
		Common_Util_HtmlParse::pair2html($tree, $length, $ext, $taglist, $html, $txtlen, $cut);
		$len = count($taglist);
		for ($i=$len-1; $i>=0; $i--)
		{
			$html .= "</".$taglist[$i].">";
		...
		if ($dropBadTags)
		{
			if (strrpos($html, ">") && strrpos($html, ">") < strrpos($html, "<"))
			{
...
if ($argc == 3)
{
	include_once("Common_Util_Str.php");

	$content = file_get_contents($argv[2]);
	if ($argv[1] == "html2tree")
	{
		$ret = Common_Util_HtmlParse::html2tree($content);
	}
	else if ($argv[1] == "getAbstract")
	{
		$ret = Common_Util_HtmlParse::getAbstract($content, "500", "...");
	}
	var_dump($ret);
}
*/
