...
class Common_Util_Image {
	...
	function copyImage($src, &$des, $strip = false, $firstgif = false) {
		...
		$needrewrite = false;
		if (($cs == Imagick::COLORSPACE_CMYK) && ($type == "jpeg" || $type == "jpg" || $type == "pjpeg")) {
	...
	function rotateImage($src, $des, $rotate) {
		$arr = pathinfo ( $src );
		if (self::$nopicallowtype [strtolower ( $arr ["extension"] )]) {
			copy ( $src, $des );
			return is_file ( $des );
	...
	function cropImageEx($src, $info, $des, $width, $height) {
	...
	function cropImage($src, $info, $des, $zoom, $left, $top, $width, $height, $fillblank = true) {
		$arr = pathinfo ( $src );
		if (self::$nopicallowtype [strtolower ( $arr ["extension"] )]) {
			copy ( $src, $des );
			return is_file ( $des );
		...
		if ($force == 1) // 满足宽
{
			if ($width >= $info ["width"]) {
				...
				return array (
						"width" => $width,
						"height" => floor ( $width * $info ["height"] / $info ["width"] )
				);
			}
		} else if ($force == 2) // 满足高
{
			if ($height >= $info ["height"]) {
				...
				return array (
						"width" => floor ( $height * $info ["width"] / $info ["height"] ),
						"height" => $height
				);
			}
		} else // 宽高都满足
			...
			if ($h > $height) //
{
	...
	// 其他 同时满足高度和宽度
	// 增加对gif图片只显示第一帧
	function convertImage($src, $info, &$des, $width, $height, $force, $bsharpen, $strip = true, $rotate = 0, $firstgif = false) {
		if ($rotate) {
	...
	function _convertRotateImage($src, $info, &$des, $width, $height, $force, $bsharpen, $strip, $rotate = 0, $firstgif = false) {
		$arr = pathinfo ( $src );
		if (self::$nopicallowtype [strtolower ( $arr ["extension"] )]) {
			copy ( $src, $des );
			return is_file ( $des );
	 ...
	 */
	function imgEffect($src, $hue, $saturation, $brightness, $contrast) {
		$image = new Imagick ();
	 ...
	 */
	function imgEffectEx($src, $colorize, $opacity) {
		$image = new Imagick ( $src );
